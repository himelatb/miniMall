...
@section('content')
    <div class="card container-fluid justify-content-centers">
      ...
      <a style="width: 150px;margin-right: 15px;" class="btn btn-success ml-auto mb-2" data-toggle="modal" data-target="#AddCmsModal" id="createNewCms">Add CMS Page</a>
      <!-- /.card-header -->
      <div class="card p-0 table-data">
        <div class="card-body">
          <table id="CmsViewTable" class="table table-bordered table-striped CmsViewTable">
          <thead>
            <tr>
              <th>#</th>
              <th>Title</th>
          ...
          <tbody>
            ...
            <tr>
              <td>{{$key+1}}</td>
                <td>{{$cmsPage['title']}}</td>
                <td>{{$cmsPage['url']}}</td>
                <td>{{date("F Y h:i A", strtotime($cmsPage['created_at']))}}</td>
                ...
                <td class="d-flex" style="border-left-width: 0px;border-top-width: 0px;border-right-width: 0px;">
                       ...
                       <div data-toggle="modal" data-target="#UpdateCmsModal" style="margin-right: 15px;" class="btn btn-primary editCmsBtn"
                         data-id="{{$cmsPage['id']}}"
                         data-title="{{$cmsPage['title']}}"
                         data-description="{{$cmsPage['description']}}"
                         data-url="{{$cmsPage['url']}}"
                         data-meta_title="{{$cmsPage['meta_title']}}"
                         data-meta_description="{{$cmsPage['meta_description']}}"
                         data-meta_keywords="{{$cmsPage['meta_keywords']}}"
                         data-status="{{$cmsPage['status']}}">
                        <svg xmlns="http://www.w3.org/2000/svg"  width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                            <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                            <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
                          </svg>
                       </div> 
                        <div  class="btn btn-danger deleteCmsBtn" data-id="{{$cmsPage['id']}}">
